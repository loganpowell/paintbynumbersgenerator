...
Generate paint-by-number images (vectorized SVG) from any input image.

> **Actively maintained fork** of [drake7707/paintbynumbersgenerator](https://github.com/drake7707/paintbynumbersgenerator) by [@loganpowell](https://github.com/loganpowell). The upstream repo is no longer maintained; this fork carries all upstream community fixes plus the new features listed below.

## Demo

Try it out → **[loganpowell.github.io/paintbynumbersgenerator](https://loganpowell.github.io/paintbynumbersgenerator/)**

## What's new in this fork

| Feature                         | Description                                                                                                                                                                                                                                 |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Crayola 64 palette preset**   | One-click load of all 64 Crayola crayon colors as color restrictions, with swatch preview                                                                                                                                                   |
| **Stable color numbers**        | Colors entered as restrictions always receive the same index number across runs                                                                                                                                                             |
| **Prune unused palette colors** | Pre-seeded restriction colors that don't appear in the image are automatically removed                                                                                                                                                      |
| **Sort palette by luminance**   | Checkbox to sort the output palette dark → light (CIE L\*), giving consistent human-readable numbering                                                                                                                                      |
| **Label font size clamping**    | Min / max pixel fields clamp label sizes — proportional to each facet's bounding box                                                                                                                                                        |
| **Single-stroke SVG export**    | "Single-stroke paths on SVG export" checkbox converts labels to Hershey Sans 1-stroke `<path>` outlines at download time — true open-path output with no font install required, compatible with Inkscape, CNC plotters, and laser engravers |
