...
export class GUIProcessManager {
  public static async process(
    ...
    if (settings.sortPaletteByLuminance) {
      ColorReducer.sortColorMapByLuminance(colormapResult);
    }
    ...
    for (const f of facetResult.facets) {
      if (f != null && f.borderSegments.length > 0) {
        ...
        if (addColorLabels) {
          ...
          const basePx =
            Math.min(f.labelBounds.width, f.labelBounds.height) *
            sizeMultiplier *
            (fontSize / 100);
          const computedFontSize = Math.min(
            fontSizeMax,
            Math.max(fontSizeMin, basePx / nrOfDigits),
          );
          ...
          const cx =
            (f.labelBounds.minX + f.labelBounds.width / 2) * sizeMultiplier;
          const cy =
            (f.labelBounds.minY + f.labelBounds.height / 2) * sizeMultiplier;
